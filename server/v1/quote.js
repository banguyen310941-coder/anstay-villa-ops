...
export default async function handler(req,res){
  ...
  const villaCode=cleanVillaCode(req.query?.villa_code),checkIn=String(req.query?.check_in||''),checkOut=String(req.query?.check_out||''),channelCode=normalizeChannel(req.query?.channel||req.headers['x-anstay-channel']||'WEBSITE'),ratePlanCode=normalizeRatePlan(req.query?.rate_plan||'BAR'),guests=intParam(req.query?.guests),extraAdults=intParam(req.query?.extra_adults),children612=intParam(req.query?.children_6_12),childrenUnder6=intParam(req.query?.children_under_6);
  ...
  try{
    ...
    if(guests>0&&pricing.villa.max_guests&&guests>Number(pricing.villa.max_guests))return sendJson(res,409,{error:'capacity_exceeded',max_guests:Number(pricing.villa.max_guests),guests});
    ...
    if(unavailable.length)return sendJson(res,409,{error:'not_sellable',dates:unavailable.map(x=>({date:x.date,blocked_by_booking:x.blocked_by_booking,blocked_by_hold:x.blocked_by_hold,blocked_by_control:x.blocked_by_control,block_type:x.block_type,stop_sell:x.stop_sell,published:x.published,rate:x.rate}))});
    ...
    sendJson(res,200,{villa:{code:pricing.villa.code,name:pricing.villa.name,max_guests:pricing.villa.max_guests==null?null:Number(pricing.villa.max_guests)},channel:{code:pricing.channel.code,name:pricing.channel.name},rate_plan:{code:pricing.plan.code,name:pricing.plan.name},check_in:checkIn,check_out:checkOut,guests:guests||null,nights,nightly,room_subtotal:roomSubtotal,surcharge:{basis:s.charge_basis||'manual',extra_adult_price:Number(s.extra_adult_price||0),child_6_12_price:Number(s.child_6_12_price||0),child_under_6_price:Number(s.child_under_6_price||0),extra_adults:extraAdults,children_6_12:children612,children_under_6:childrenUnder6,calculated_total:surchargeTotal,note:s.note||null},grand_total:grandTotal,currency:pricing.plan.currency||'VND',requires_manual_surcharge_confirmation:requiresManual});
